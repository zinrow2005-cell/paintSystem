# 小小畫家學習樂園 V1.7.35｜真正全螢幕畫布與大字練習版

- 修正 V1.7.33 在 iPad 全螢幕時 Canvas 被壓成底部細條、無法繪畫的問題。
- 全螢幕改為獨立 viewport shell，不再把原工作卡硬改 Grid。
- 畫畫／寫字／發音仍可在全螢幕上方切換。
- iPad Safari 依 visualViewport 即時計算可視高度與畫布 4:3 尺寸。
- 全螢幕固定提供「✕ 離開」按鈕。

# 小小畫家學習樂園 V1.7.34｜全螢幕畫板與工具列穩定修正版
...
# 小小畫家學習樂園 V1.7.34｜GitHub Pages 正式上線版
...
## 更新注意
若 GitHub 上傳新版後 iPad 仍看到舊畫面，先重新整理；主畫面 PWA 若仍持有舊快取，可完全關閉後重開。V1.7.34 的 Service Worker cache key 已獨立版本化。
...
## V1.7.35 重點修正
- 全螢幕「畫畫」改成真正 100% viewport 畫布；模式、教材名稱、離開鍵、調色盤、粗細／復原／收藏都只以浮動控制列疊在畫布上，不再切割畫布空間。
- 全螢幕只保留「畫畫／寫字」兩種模式；發音不再搬入全螢幕。一般頁面的英文／中文發音功能仍保留。
- 全螢幕「寫字」練習簿鋪滿整個 viewport，英文、中文、注音會依字數與螢幕尺寸自動放大，並保留適合手寫的間距。
- iPad／手機／桌機使用同一套 viewport overlay，控制元件不參與 Canvas 版面計算。
